/*
=========================================================
TÍTULO: ZNT - Configuração de Textos do Site
ÚLTIMA MUDANÇA: Conversão de todas as unidades de medida de tamanho de fonte (tamanho) de 'rem' para 'px', facilitando a edição e o controle exato do design pelo usuário.
DATA E HORA: 08 de Setembro de 2026, 20:15
VERSÃO: 11.2
=========================================================
*/

const TEXTOS = {
    // Menu
    nav_sobre: { texto: "Sobre nós", tamanho: "16px" },
    nav_contatos: { texto: "Contatos", tamanho: "16px" },

    // Hero (Seção Principal)
    hero_tag: { texto: "EXCELÊNCIA EM TECNOLOGIA", tamanho: "12px" },
    hero_titulo: { texto: "Conectando você à solução!", tamanho: "64px" },
    hero_destaque: { texto: "de alto desempenho", tamanho: "64px" },
    hero_descricao: { texto: "Somos especialistas em manutenção avançada, otimização térmica, recuperação de dados e setups de alta performance. Profissionais prontos para entregar a melhor solução para sua necessidade.", tamanho: "18px" },
    btn_proposta: { texto: "Solicitar proposta", tamanho: "16px" },

    // Nomes dos Filtros
    filtro_todos: { texto: "Todos os Serviços", tamanho: "16px" },
    filtro_hardware: { texto: "Hardware", tamanho: "16px" },
    filtro_software: { texto: "Software", tamanho: "16px" },
    filtro_dados: { texto: "Rede & Dados", tamanho: "16px" },

    // Modal de Solicitação
    modal_titulo: { texto: "Excelente escolha!", tamanho: "24px" },
    modal_subtitulo: { texto: "Você selecionou o serviço de", tamanho: "16px" },
    modal_aviso: { texto: "Para garantir um atendimento rápido e personalizado, finalizaremos os detalhes e dúvidas diretamente com você.", tamanho: "14px" },
    modal_btn_avancar: { texto: "Avançar para o WhatsApp", tamanho: "16px" },
    modal_btn_cancelar: { texto: "Cancelar e voltar ao catálogo", tamanho: "14px" }
};
